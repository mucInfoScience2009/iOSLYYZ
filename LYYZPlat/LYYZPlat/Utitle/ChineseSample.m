...
#define HANZI_START_NEW 19968
#define HANZI_COUNT_NEW 20902

static char firstNewLetterArray[HANZI_COUNT_NEW] =
...
char pinyinFirstNewLetter(unsigned short hanzi) {
	int index = hanzi - HANZI_START_NEW;
	if (index >= 0 && index <= HANZI_COUNT_NEW) {
		return firstNewLetterArray[index];
	} else {
		return '#';
	}
}

@implementation ChineseSample
	...
	if( !string || 0 == [string length] )
		...
		if(([string characterAtIndex:0] > 64 && [string characterAtIndex:0] < 91) || 
		   ([string characterAtIndex:0] > 96 && [string characterAtIndex:0] < 123) ) {
			...
			cLetter = pinyinFirstNewLetter((unsigned short)[string characterAtIndex:0]);
